...
extension StringUtils on String {
  ...
  String wrapText([int wrapLength = 76]) {
    ...
    for (final word in words) {
      final wordWidth = word.displayWidth;

      if (lineWidth + wordWidth <= wrapLength) {
      ...
      for (final char in characters) {
        ...
        if (lineWidth + charWidth <= wrapLength) {
          textLine.write(char);
          lineWidth += charWidth;
        } else {
          String? leftChar;
          if (lastChar != null &&
              lastChar._isEnglishLetter &&
              char._isEnglishLetter) {
            final lastLine = textLine.toString();
            textLine.clear();
            textLine.write(lastLine.substring(0, lastLine.length - 1));
            textLine.write('-');
            leftChar = lastChar;
          }
  ...
  int get displayWidth {
    ...
    text = text.stripEscapeCharacters();
    if (text.isEmpty) {
      return 0;
    }
    ...
    for (final character in text.characters) {
      width += _charDisplayWidth(character);
    }
    return width;
  }
  ...
  /// Returns true if this character is an English letter.
  bool get _isEnglishLetter =>
      length == 1 && codeUnitAt(0) >= 0x41 && codeUnitAt(0) <= 0x5A ||
      codeUnitAt(0) >= 0x61 && codeUnitAt(0) <= 0x7A;
}

RegExp get _emojiRegex => RegExp(
...
///
/// Fullwidth and wide East Asian characters count as two columns, control and
/// combining characters count as zero columns, and all other characters count
/// as one column. Ambiguous East Asian width characters count as one column
/// when [ambiguousIsNarrow] is true, or two columns otherwise.
int _charDisplayWidth(String character, {bool ambiguousIsNarrow = true}) {
  if (character.isEmpty) {
    return 0;
  }
  var codePoint0 = character.codeUnitAt(0);
  // Ignore control characters
  if (codePoint0 <= 0x1F || (codePoint0 >= 0x7F && codePoint0 <= 0x9F)) {
    return 0;
  }
  // Ignore combining characters
  if (codePoint0 >= 0x300 && codePoint0 <= 0x36F) {
    return 0;
  }
  var codePoint1 = (character.length == 2) ? character.codeUnitAt(1) : 0;
  var codePoint = codePoint0;
  if ((0xD800 <= codePoint0 && codePoint0 <= 0xDBFF) &&
      (0xDC00 <= codePoint1 && codePoint1 <= 0xDFFF)) {
    codePoint0 &= 0x3FF;
    codePoint1 &= 0x3FF;
    codePoint = codePoint0 | codePoint1;
    codePoint += 0x10000;
  }
